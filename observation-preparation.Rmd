...
amHighVolCarData <- addCarLinks(carData = "./amHighVolCarData.sqlite", layer = "amhighvolcardata")
st_write(amHighVolCarData, "amHighVolCarData.sqlite", delete_dsn = T)
...
# ggplot() + 
#   annotation_map_tile(type="osmgrayscale",zoom=9, alpha=0.6) + 
#   geom_sf(data = amHighVolCarData, colour = 'red') 
...
```{r}
pmHighVolCarData <- addCarLinks(carData = "./pmHighVolCarData.sqlite", layer = "pmhighvolcardata")
st_write(pmHighVolCarData, "pmHighVolCarData.sqlite", delete_dsn = T)
...
# ggplot() + 
#   annotation_map_tile(type="osmgrayscale",zoom=9, alpha=0.6) + 
#   geom_sf(data = pmHighVolCarData, colour = 'red') 
...
walkDataCoordinated <- walkDataJoined %>% 
  ...
  st_transform(28355) %>% 
  dplyr::select(ID,Sensor_ID, Sensor_Name, Hourly_Counts, direction_1, direction_2, Time, geometry)
...
rail.lines <- st_read("./generatedNetworks/MATSimMelbNetwork.sqlite", layer = "links", quiet=T) %>%

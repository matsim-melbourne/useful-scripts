...
paths <- dir_ls("./data/sim-outputs/",glob = "*.txt" )
if (length(paths)>0) {
...
#!/bin/bash
if [[ $# -eq 0 ]]; then
  events_file=./data/sim-outputs/output_events.xml.gz;
else
  events_file=$1
fi
...
cut -d "\"" -f 2,4,6,8,10 |
sed -e "s/\"/,/g"|
sed -e '1i\'$'\n''time,type,person,link,legMode' | cat > ./data/sim-outputs/person_trip.txt
...
cut -d "\"" -f 2,4,6,8 |
sed -e "s/\"/,/g"|
sed -e '1i\'$'\n''time,type,link,vehicle' | cat > ./data/sim-outputs/trip_links.txt
...
sed -e "s/\"/,/g"|
sed -e '1i\'$'\n''time,type,person,link,vehicle,networkMode' | cat > ./data/sim-outputs/vehicle_trip.txt
...
cut -d "\"" -f 2,4,6,8,10 |
sed -e "s/\"/,/g"|
sed -e '1i\'$'\n''time,type,person,link,actType' | cat > ./data/sim-outputs/person_act.txt
...
tripLinks <- vroom("./data/sim-outputs/trip_links.txt")
# tripLinks %>% slice_sample(prop=0.1) %>% write_delim("./data/sim-outputs/trip_links_small.txt")
# tripLinks <- vroom("./data/sim-outputs/trip_links_small.txt") 
...
vehicleTrips <- read_csv("./data/sim-outputs/vehicle_trip.txt")
...
if (file.exists("./data/networks/networkConverted.sqlite")) {
  ...
  networkConverted <- convertXMLNetowrk(xmlFile = "./data/sim-outputs/output_network.xml",
                                        netCRS = 28355)
...
networkLinksWithDailyVol_car %>% 
  ...
  annotation_map_tile(zoom=9, alpha=0.6) +
...
# carExitLinkAggregatedHourly <- tempDf %>% 
  filter(networkMode=="car") %>% # filter to car trips only
  mutate(id=as.character(link)) %>% 
  mutate(leftHour=paste0("h",leftHour)) %>% 
...
Joining Hourly netwalk traffic data to road network

```{r joining Hourly netwalk volumes to network}

netwalkExitLinkAggregatedHourly <- exitLinksWithTime %>%
# bikeExitLinkAggregatedHourly <- tempDf %>% 
  filter(networkMode=="netwalk") %>% # filter to car trips only
    mutate(leftHour=paste0("h",leftHour)) %>% 
  mutate(id=as.character(link)) %>% 
  group_by(id, leftHour) %>% 
  summarise(hourly_vol=n()) %>% 
...
networkLinksWithHourlyVol_netwalk <- networkLinks %>% 
  left_join(netwalkExitLinkAggregatedHourly, by = "id")   

st_write(networkLinksWithHourlyVol_netwalk, 
         "./data/simOutputJoined/networkLinksWithHourlyVol.sqlite",
         layer="netwalk",
         delete_layer = T)
```
plotting Hourly netwalk traffic volume

```{r plotting Hourly netwalk volumes on network}

# networkLinksWithHourlyVol$hourly_vol <- st_read("networkLinksWithHourlyVol.sqlite")

networkLinksWithHourlyVol_netwalk %>% 
  drop_na(total_vol) %>% 
  # filter(hourly_vol>10) %>% 
  ggplot() +
  # annotation_map_tile(type="osmgrayscale",zoom=9, alpha=0.6) +

...
if (file.exists("./data/networks/networkConverted.sqlite")) {
  ...
  networkConverted <- convertXMLNetowrk(xmlFile = "./data/sim-outputs/output_network.xml.gz",
                                        netCRS = 28355)
...
amHighVolCarData %>% 
  st_centroid() %>% 
  ggplot() + 
  annotation_map_tile(zoom=10, alpha=0.6) +
...
cyclingObsJoined2Network %>% 
  ...
  annotation_map_tile(zoom=10, alpha=0.8) +
  geom_sf(aes(color=total_vol))  
```
...
walkDataCoordinated %>% 
  ...
  annotation_map_tile(zoom=12, alpha=0.8) +
  geom_sf(aes(color=total_vol))  
```

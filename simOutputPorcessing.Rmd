...
Joining daily data to road network

```{r joining daily volumes to network}

tripLinkAggregatedDaily <- tripLinksWithTime %>% 
  mutate(id=as.character(link)) %>% 
  group_by(id) %>% 
  summarise(total_vol=n())

networkLinksWithDailyVol <- networkLinks %>% left_join(tripLinkAggregatedDaily, by = "id")   
st_write(networkLinksWithDailyVol, "./data/simOutputJoined/networkLinksWithDailyVol.sqlite", delete_layer = T)
```

plotting daily volume

```{r plotting daily volumes on network}

# networkLinksWithDailyVol$total_vol <- st_read("networkLinksWithDailyVol.sqlite")

networkLinksWithDailyVol %>% 
  drop_na(total_vol) %>% 
  # filter(total_vol>10) %>% 
  ggplot() +

...
```{r cleaning old files, eval=FALSE}
...
```{bash processing event XMLs, eval=FALSE}
...
tripLinks <- vro("./data/simOutput/trip_links.txt") 
tripLinksSampled <- tripLinks %>% slice_sample(prop=0.1) 
...
We can either try to join to get the duration on each link, however, it will result in missing many:
...
Instead we only consider link exit event as our messure for count:
...
```{r checking how many we miss to match}
...
tripLinksWithTime <- tripLinksFiltered %>% 
  mutate(enterHour=  floor(enteredTime / 3600)) %>% 
  mutate(leftHour=  floor(leftTime / 3600)) %>% 
  mutate(enterHMS=paste(sprintf("%02d",enterHour),
...
#### Aggregating entering link event by the hour of the day

```{r plotting link enter events}

tripLinksWithTime %>% 
  group_by(enterHour) %>% 
  summarise(n=n()) %>% 
  ggplot(aes(x=enterHour, y=n)) +
  geom_col() +
  labs(title = "Aggregated entering link event count by the hour of the day") +
  xlab("Hour of the day") +
  ylab("Link enter even count")
...
```{r joining daily car volumes to network}

carTripLinkAggregatedDaily <- tripLinksWithTime %>% 
...
networkLinksWithDailyVol_car <- networkLinks %>% 
  left_join(carTripLinkAggregatedDaily, by = "id")   
...
```{r plotting daily car volumes on network}
...
```{r joining daily bike volumes to network}

bikeTripLinkAggregatedDaily <- tripLinksWithTime %>% 
...
networkLinksWithDailyVol_bike <- networkLinks %>% 
  left_join(bikeTripLinkAggregatedDaily, by = "id")   
...
```{r plotting daily bike volumes on network}
